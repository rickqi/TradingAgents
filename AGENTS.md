...
| `data_vendors` | 各项 `"twelve_data,yfinance"` | 按类别覆盖。选项：`yfinance`, `alpha_vantage`, `tencent_sina`, `akshare`, `twelve_data`。支持逗号分隔降级链。`sentiment_data` 默认 `"akshare"`，`opencli_market` 默认 `"opencli"`。 |
